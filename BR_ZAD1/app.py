...
class Boardd:
    ...
    def make_child(self, board_after_move, move):
        child = Boardd(board_after_move, self, move, self.way)
    ...
    def make_move(self, move):
        ...
        if move == 'L':
            ...
            self.make_child(tmp_array, move)
        elif move == 'R':
            tmp_array = []
            for row in self.board:
                tmp_array.append(row.copy())
...
# Auxiliary functions
def change_blank_field_pos(last_move):
    if last_move == 'U':
        EMPTY_FIELD['row'] += 1
    if last_move == 'D':
        EMPTY_FIELD['row'] -= 1
...
def dfs(start_time):
    ...
    current_node = Boardd(START_BOARD, 'Root', None, [])
    root_flag = True
    parent_flag = False
    max_depth = False
    depth_level = 0
    ...
    while True:
        ...
        if is_solved(current_node.board, SOLVED_BOARD):
            ...
            for move in current_node.to_visit:
                ...
                change_blank_field_pos(last_move)
                current_node = current_node.parent
            try:
